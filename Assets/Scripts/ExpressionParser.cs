...
namespace LinqyCalculator
{
    ...
    {
        public static double x = 0.0, y = 0.0;
        ...
        static Expression CallFunction(string name, Expression[] parameters)
        {
            ...
            var methodInfo = typeof(Math)
                .GetTypeInfo().GetMethod(new string(cname), parameters.Select(e => e.Type).ToArray());
        ...
        static readonly Parser<Expression> Variable =
            from name in Parse.Char('x').Or(Parse.Char('y'))
            select CallVariable(name);
        ...
        static Expression CallVariable(char var)
        {
            MethodInfo methodInfo = null;

            if (var == 'x')
            {
                methodInfo = typeof(ExpressionParser)
                    .GetTypeInfo().GetMethod("GetX");
            }
            else if (var == 'y')
            {
                methodInfo = typeof(ExpressionParser)
                    .GetTypeInfo().GetMethod("GetY");
            }

            if (methodInfo == null)
            {
                throw new ParseException(string.Format("Variable '{0}' does not exist.", var));
            }
